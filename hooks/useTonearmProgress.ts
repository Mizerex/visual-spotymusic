...
const LAST_GROOVE_ANGLE = 31;
const INTRA_TRACK_TRAVEL = 0.55;
...
export function useTonearmProgress(position: number, duration: number, hasTrack: boolean, queueIndex: number, queueLength: number) {
  return useMemo(() => {
    ...
    const visualTrackIndex = Math.max(0, queueIndex) % TRACKS_PER_VISUAL_VINYL;
    const vinylProgress = queueLength > 0
      ? Math.min(1, (visualTrackIndex + trackProgress * INTRA_TRACK_TRAVEL) / (TRACKS_PER_VISUAL_VINYL - 1))
      : trackProgress;
