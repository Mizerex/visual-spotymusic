...
export function Turntable() {
  const { playback, toggle } = useSpotifyAuth();
  ...
  return <section className="turntable-section" aria-label="Toca-discos">
    ...
    <div className="deck">
      ...
      <div className="platter"><div className="platter-dots" /><Vinyl playing={playback.isPlaying} rpm={rpm} /></div>
      <Tonearm position={playback.position} duration={playback.duration} hasTrack={Boolean(playback.track)} />
      <div className="deck-controls">
        <button className="power-knob" onClick={toggle} aria-label={playback.isPlaying ? "Pausar" : "Iniciar reprodução"}><i className={playback.isPlaying ? "on" : ""} /></button>
        ...
        <span className="pitch-label">QUARTZ LOCK</span>
      </div>
    </div>
