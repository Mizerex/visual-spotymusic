...
export function NowPlaying() {
  ...
  const showArtwork = Boolean(artwork?.trim()) && !artFailed;
  return <aside className="now-playing">
    ...
    <div className={`album-art ${showArtwork ? "has-image" : ""}`}>
      {showArtwork ? <img src={artwork} alt={`Capa de ${track?.album.name || "álbum"}`} onError={() => setArtFailed(true)} /> : <><div className="fallback-art"><span>VISUAL</span><b>♫</b><small>SPOTYMUSIC SESSIONS</small></div></>}
